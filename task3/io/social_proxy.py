...
PLATFORM_KEYS = ["instagram", "tiktok", "twitter", "youtube", "x"]
...
def _detect_platform(col: str) -> Optional[str]:
    lower = col.lower()
    for key in PLATFORM_KEYS:
        if key in lower:
            if key == "x" and "twitter" in lower:
                return "twitter"
            return "twitter" if key == "x" else key
    return None


def _is_celebrity_column(col: str) -> bool:
    lower = col.lower()
    return "celebrity" in lower or "celeb" in lower


def _is_partner_column(col: str) -> bool:
    lower = col.lower()
    return "partner" in lower or "pro" in lower
...
def load_social_proxy_map(path: str) -> Dict[Tuple[int, str, str], Dict[str, object]]:
    ...
    for row in rows:
        ...
        for col, val in row.items():
            plat = _detect_platform(col)
            if plat is None:
                continue
            raw_val = _parse_number(val)
            if raw_val is None:
                continue
            if _is_celebrity_column(col):
                rec[f"P_cele_{plat}"] = math.log1p(raw_val) if raw_val > 0 else None
                rec[f"missing_cele_{plat}"] = raw_val <= 0
            elif _is_partner_column(col):
                rec[f"P_partner_{plat}"] = math.log1p(raw_val) if raw_val > 0 else None
                rec[f"missing_partner_{plat}"] = raw_val <= 0
...
def save_social_proxy_csv(proxy_map: Dict[Tuple[int, str, str], Dict[str, object]], out_path: Path) -> None:
    ...
    # include any platform columns
    extra_cols = sorted({
        key for row in rows for key in row.keys()
        if key.startswith("P_cele_") or key.startswith("P_partner_") or key.startswith("missing_cele_") or key.startswith("missing_partner_")
    })
